...
/* A plugin exports the following functions.  */

/* Configure the plugin.  Return an opaque pointer that will be used for successive calls.  */
typedef int (*run_oci_seccomp_notify_start)(void **opaque, struct libcrun_load_seccomp_notify_conf_s *conf, size_t size_configuration);
/* Try to handle a single request.  It MUST be defined.
   ...
   1: sresp filled and ready to be notified to seccomp.
   2: the notification will be handled internally by the plugin and forwarded to seccomp_fd. It is useful for asynchronous handling.
*/
typedef int (*run_oci_seccomp_notify_handle_request)(void *opaque, struct seccomp_notif *sreq, struct seccomp_notif_resp *sresp, int seccomp_fd, int *handled);
/* Stop the plugin.  The opaque value is the return value from run_oci_seccomp_notify_start.  */
typedef int (*run_oci_seccomp_notify_stop)(void *opaque);
/* Retrieve the API version used by the plugin.  It MUST return 1. */
typedef int (*run_oci_seccomp_notify_plugin_version)();

struct plugin
{
  void *handle;
  void *opaque;
  run_oci_seccomp_notify_handle_request handle_request_cb;
};
...
libcrun_load_seccomp_notify_plugins (struct seccomp_notify_context_s **out, const char *plugins, struct libcrun_load_seccomp_notify_conf_s *conf, libcrun_error_t *err)
{
  ...
  if (seccomp (SECCOMP_GET_NOTIF_SIZES, 0, &ctx->sizes) < 0)
  ...
  for (s = 0, it = strtok_r (b, ":", &saveptr); it; s++, it = strtok_r (NULL, ":", &saveptr))
    {
      run_oci_seccomp_notify_plugin_version version_cb;
      run_oci_seccomp_notify_start start_cb;
      void *opq = NULL;
      ...
      version_cb = (run_oci_seccomp_notify_plugin_version) dlsym (ctx->plugins[s].handle, "run_oci_seccomp_notify_version");
      if (version_cb != NULL)
        {
          int version;
      ...
      ctx->plugins[s].handle_request_cb = (run_oci_seccomp_notify_handle_request) dlsym (ctx->plugins[s].handle, "run_oci_seccomp_notify_handle_request");
      ...
      start_cb = (run_oci_seccomp_notify_start) dlsym (ctx->plugins[s].handle, "run_oci_seccomp_notify_start");
      if (start_cb)
        {
          int ret;
...
libcrun_seccomp_notify_plugins (struct seccomp_notify_context_s *ctx, int seccomp_fd, libcrun_error_t *err)
{
      ...
      if (ctx->plugins[i].handle_request_cb)
        {
          ...
          ret = ctx->plugins[i].handle_request_cb (ctx->plugins[i].opaque, ctx->sreq, ctx->sresp, seccomp_fd, &handled);
          if (UNLIKELY (ret != 0))
 ...
 send_resp:
  ...
  return ioctl (seccomp_fd, SECCOMP_IOCTL_NOTIF_SEND, ctx->sresp);
#else
  return crun_make_error (err, ENOTSUP, "seccomp notify support not available");
#endif
}
...
libcrun_free_seccomp_notify_plugins (struct seccomp_notify_context_s *ctx, libcrun_error_t *err)
{
  ...
  for (i = 0; i < ctx->n_plugins; i++)
    if (ctx->plugins && ctx->plugins[i].handle)
      {
        run_oci_seccomp_notify_stop cb;

        cb = (run_oci_seccomp_notify_stop) dlsym (ctx->plugins[i].handle, "run_oci_seccomp_notify_stop");
        if (cb)
